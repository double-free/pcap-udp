...
void Writer::write_order(const md::Order &order, uint64_t pcap_ts, uint64_t pcap_seq)
{
    ...
    csv_file_ << pcap_ts << ',' << pcap_seq << ",2,1,0," << order.transaction_time() % 100000000 << ','
              ...
              << order.price() << ',' << order.quantity() / Writer::QUANTITY_MULT << '\n';
}

void Writer::write_trade(const md::Trade &trade, uint64_t pcap_ts, uint64_t pcap_seq)
{
...
void Writer::write_snapshot(const md::SnapshotWrapper &snapshot, uint64_t pcap_ts, uint64_t pcap_seq, int depth)
{
    // placeholders
    csv_file_ << "09:42:12.094767,1587606132095370,23994,"
              << pcap_ts << ',' << pcap_seq << ",24,"
              << snapshot.security_id << ",SZ," << snapshot.orig_time << ','
              << snapshot.total_trade_volume / Writer::QUANTITY_MULT << ','
              << 1.0 * snapshot.total_trade_value / Writer::AMOUNT_MULT << ','
              << 1.0 * snapshot.latest_trade_price / Writer::MD_PRICE_MULT << ",0,";
